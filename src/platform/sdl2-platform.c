...
static boolean pollBrogueEvent(rogueEvent *returnEvent, boolean textInput) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_QUIT) {
            ...
            androidSetRestoringVisible(true);
            androidResetTouchState();
            SDL_FlushEvents(SDL_KEYDOWN, SDL_KEYUP);
            SDL_FlushEvents(SDL_FINGERDOWN, SDL_FINGERMOTION);
            SDL_Log("Brogue lifecycle: native event loop entered background");
...
static void _delayUpTo(short ms) {
    if (androidAppInBackground && ms < 50) {
        ms = 50;
    }
...
static void _nextKeyOrMouseEvent(rogueEvent *returnEvent, boolean textInput, boolean colorsDance) {
    ...
    while (true) {
        ...
        if (colorsDance && now >= nextColorDanceTime) {
            ...
            // Preserve the original 36 ms cadence without replaying missed
            // updates in a burst after a stall or lifecycle transition.
            nextColorDanceTime += ((now - nextColorDanceTime)
                / TERRAIN_COLOR_DANCE_INTERVAL_MS + 1)
                * TERRAIN_COLOR_DANCE_INTERVAL_MS;
        }
