...
// Visual-only horizontal position of the left sidebar. Positive moves right.
// Sidebar touch coordinates intentionally do not follow this offset.
...
static float cameraFollowAlpha(float referenceAlpha, double elapsedMs) {
    if (elapsedMs <= 0.0) return 0.0f;

    // Preserve the old 36 ms follow speed while rendering at any refresh rate.
...
static int sidebarOffsetPixels(int rowCenterY, float cellWidth) {
    int offset = (int)roundf(androidSidebarXOffsetCells * cellWidth);
    int radius = androidTopLeftCornerRadiusPx;
    int centerX = androidTopLeftCornerCenterXPx;
    int centerY = androidTopLeftCornerCenterYPx;

    if (radius > 0 && rowCenterY < centerY) {
        float dy = (float)rowCenterY - centerY;
        float inside = (float)radius * radius - dy * dy;
        if (inside > 0.0f) {
            int safeX = (int)ceilf(centerX - sqrtf(inside));
            offset = max(offset, safeX);
        }
    }
    return offset;
}
...
void setRenderMode(enum RenderMode mode) {
    ...
    if (mode == RENDER_GAMEPLAY) {
        clearOverlayRegion();
        cameraLastUpdateCounter = 0;
    } else if (mode == RENDER_TITLE)
        memset(uiTiles, 0, sizeof(uiTiles));
    else if (mode == RENDER_LOADING)
        memset(loadingTiles, 0, sizeof(loadingTiles));
}
...
void setLoadingProgress(unsigned long amount, unsigned long maximum) {
    ...
    const int barWidth = (int)sizeof(label) - 1;
    const int startX = (COLS - barWidth) / 2;
    const int row = ROWS / 2;
    double filled = maximum > 0
        ? (double)barWidth * min(amount, maximum) / (double)maximum
        : 0.0;
    int boundary = (int)floor(filled);
    short boundaryDarken = (short)round(75.0 * (1.0 - (filled - boundary)));

    memset(loadingTiles, 0, sizeof(loadingTiles));
    for (int i = 0; i < barWidth; i++) {
        color background = darkPurple;
        if (i > boundary) {
            applyColorAverage(&background, &black, 75);
        } else if (i == boundary && boundary < barWidth) {
            applyColorAverage(&background, &black, boundaryDarken);
        }

        color foreground = white;
        applyColorAverage(&foreground, &background, 33);
        loadingTiles[row][startX + i] = (ScreenTile){
            .foreRed = foreground.red,
            .foreGreen = foreground.green,
            .foreBlue = foreground.blue,
            .backRed = background.red,
            .backGreen = background.green,
            .backBlue = background.blue,
            .charIndex = fontIndex(label[i]),
            .needsRefresh = 1
        };
    }
...
static SDL_atomic_t rendererRecoveryPending;
...
///
/// Android always uses one large texture with linear interpolation so pinch
/// zoom never rebuilds four CPU-downscaled atlases. Other platforms switch to
/// that path only when screen tiles would exceed 64x64 pixels.
///
...
int consumeRendererRecoveryRequest(void) {
    return SDL_AtomicCAS(&rendererRecoveryPending, 1, 0);
}
...
static void createTextures(SDL_Renderer *renderer, int outputWidth, int outputHeight) {
    ...
    // This size depends on the viewport aspect ratio, not the pinch-zoom scale,
    // so the atlas remains reusable throughout a zoom gesture.
    int newBaseTileWidth = max(1, min(TILE_WIDTH, round(TILE_HEIGHT * tileAspectRatio)));
    int newBaseTileHeight = max(1, min(TILE_HEIGHT, round(TILE_WIDTH / tileAspectRatio)));

    // Older mobile GPUs can advertise or attempt very large textures but fail
    // when the backing surface and texture consume too much memory. Keep the
    // Android atlas within both the renderer's reported limits and a 4096px
    // compatibility ceiling, scaling both axes together to preserve glyphs.
    SDL_RendererInfo rendererInfo;
    int maxAtlasWidth = 4096;
    int maxAtlasHeight = 4096;
    if (SDL_GetRendererInfo(renderer, &rendererInfo) == 0) {
        if (rendererInfo.max_texture_width > 0) {
            maxAtlasWidth = min(maxAtlasWidth, rendererInfo.max_texture_width);
        }
        if (rendererInfo.max_texture_height > 0) {
            maxAtlasHeight = min(maxAtlasHeight, rendererInfo.max_texture_height);
        }
    }
    double atlasScale = min(1.0,
        min((double)maxAtlasWidth / (newBaseTileWidth * TILE_COLS),
            (double)maxAtlasHeight / (newBaseTileHeight * TILE_ROWS)));
    newBaseTileWidth = max(1, (int)floor(newBaseTileWidth * atlasScale));
    newBaseTileHeight = max(1, (int)floor(newBaseTileHeight * atlasScale));
#else
...
///
void updateScreen() {
    if (!Win || androidAppInBackground) return;

    SDL_Renderer *renderer = SDL_GetRenderer(Win);
    if (!renderer) {
        Uint32 rendererFlags = SDL_RENDERER_PRESENTVSYNC;
        if (softwareRendering) rendererFlags |= SDL_RENDERER_SOFTWARE;
    ...
    // Loading is fixed screen-space UI, independent of gameplay camera pan and zoom.
    if (renderMode == RENDER_LOADING) {
    ...
    if (inGame && !androidCameraDetached) {
        ...
        // Near map edges the player cannot physically be screen-centered.
        // Compare against the nearest reachable pan so fast recenter can end.
        float maxPanX = fmaxf(0.0f, (zoomW - screenW) / 2.0f);
        float maxPanY = fmaxf(0.0f, (zoomH - screenH) / 2.0f);
        targetPanX = fmaxf(-maxPanX, fminf(maxPanX, targetPanX));
        targetPanY = fmaxf(-maxPanY, fminf(maxPanY, targetPanY));
        ...
        if (androidCameraSnap) {
            ...
            if (androidCameraFastRecenter
                    && fabsf(targetPanX - androidPanX) < 0.5f
                    && fabsf(targetPanY - androidPanY) < 0.5f) {
                androidPanX = targetPanX;
                androidPanY = targetPanY;
                androidCameraFastRecenter = false;
            }
        ...
        for (int step = -1; step < numTextures; step++) {
            for (int x = 0; x < COLS; x++) {
                ...
                for (int y = 0; y < ROWS; y++) {
                    ...
                    int rowDestX = destX;
                    if (x < STAT_BAR_WIDTH) {
                        rowDestX += sidebarOffsetPixels(
                            destY + tileHeight / 2, (float)uiW / COLS);
                    } else if (x >= STAT_BAR_WIDTH
                            && (messageArchiveActive
                                || y < MESSAGE_LINES || y == ROWS - 2)) {
                        rowDestX += (int)roundf(
                            androidMessageTextXOffsetCells * uiW / COLS);
                    }

                    if (step < 0) {
                        if (tile->backRed == 0 && tile->backGreen == 0 && tile->backBlue == 0)
                            continue;
                        SDL_Rect dest = {rowDestX, destY, tileWidth, tileHeight};
                        if (SDL_SetRenderDrawColor(renderer,
                            round(2.55 * tile->backRed),
                            round(2.55 * tile->backGreen),
                            round(2.55 * tile->backBlue), 255) < 0) sdlfatal(__FILE__, __LINE__);
