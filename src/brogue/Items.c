...
typedef enum {
    ANDROID_INVENTORY_FILTER_NONE,
    ANDROID_INVENTORY_FILTER_QUICK_TARGET,
} AndroidInventorySelectionFilter;
...
static boolean itemIsQuickTargetItem(const item *theItem) {
    return theItem
        && (itemIsThrowingWeapon(theItem)
            || theItem->category == STAFF
            || theItem->category == WAND);
}

static short quickTargetItemRank(const item *theItem) {
    if (itemIsThrowingWeapon(theItem)) return 0;
    if (theItem && theItem->category == STAFF) return 1;
    if (theItem && theItem->category == WAND) return 2;
    return 3;
}

static boolean androidInventoryItemMatchesSelectionFilter(const item *theItem) {
    switch (androidInventorySelectionFilter) {
        case ANDROID_INVENTORY_FILTER_QUICK_TARGET:
            return itemIsQuickTargetItem(theItem);
        case ANDROID_INVENTORY_FILTER_NONE:
        default:
            return true;
    }
}

static short numberOfQuickTargetPackItems(boolean displayErrors) {
    item *theItem;
    ...
    for (theItem = packItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
        if (itemIsQuickTargetItem(theItem)) {
            matchingItemCount++;
        }
    }

    if (matchingItemCount == 0 && displayErrors) {
        confirmMessages();
        message("You have no projectiles, staffs or wands.", 0);
    }
    return matchingItemCount;
}

static item *promptForQuickTargetItem(void) {
    item *theItem;

    if (!numberOfQuickTargetPackItems(true)) {
        return NULL;
    }

    androidInventorySelectionFilter = ANDROID_INVENTORY_FILTER_QUICK_TARGET;
    androidInventoryHideUnselectable = true;
    theItem = promptForItemOfType((ALL_ITEMS), 0, 0,
                                  KEYBOARD_LABELS ? "Target with what? (a-z, shift for more info; or <esc> to cancel)" : "Target with what?",
                                  true);
    androidInventorySelectionFilter = ANDROID_INVENTORY_FILTER_NONE;
    androidInventoryHideUnselectable = false;
    return theItem;
}
...
void pickUpItemAt(pos loc) {
    ...
    if (numberOfItemsInPack() < MAX_PACK_ITEMS || (theItem->category & GOLD) || itemWillStackWithPack(theItem)) {
        ...
        sprintf(buf, "you now have %s.", buf2);
        messageWithColor(buf, &itemMessageColor, 0);
...
char displayInventory(unsigned short categoryMask,
    ...
    itemNumber = 0;
    if (rogue.weapon) { itemList[itemNumber++] = rogue.weapon; }
    if (rogue.armor)  { itemList[itemNumber++] = rogue.armor; }
    if (rogue.ringLeft)  { itemList[itemNumber++] = rogue.ringLeft; }
    if (rogue.ringRight) { itemList[itemNumber++] = rogue.ringRight; }
    for (theItem = packItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
        if (!(theItem->flags & ITEM_EQUIPPED)) {
    ...
    // The Target action presents projectiles first, followed by staffs and
    // wands, independently of their inventory-letter order.
    if (!waitForAcknowledge
        && androidInventorySelectionFilter == ANDROID_INVENTORY_FILTER_QUICK_TARGET) {
        for (i = 1; i < itemNumber; i++) {
            item *candidate = itemList[i];
            short candidateRank = quickTargetItemRank(candidate);
            short j = i;
            while (j > 0 && quickTargetItemRank(itemList[j - 1]) > candidateRank) {
                itemList[j] = itemList[j - 1];
                j--;
            }
            itemList[j] = candidate;
        }
    }
    ...
    for (i = 0; i < itemNumber; i++) {
        ...
        if (!waitForAcknowledge) {
            selectable = selectable && androidInventoryItemMatchesSelectionFilter(theItem);
            if (androidInventoryHideUnselectable && !selectable) {
                continue;
            }
        }

        int magicPol = displayMagicCharForItem(theItem) ? itemMagicPolarity(theItem) : 0;
        int iconRed = clamp(theItem->foreColor->red, 0, 100) * 255 / 100;
        int iconGreen = clamp(theItem->foreColor->green, 0, 100) * 255 / 100;
    ...
    *targetConfirmed = *canceled = *tabKey = false;
    ...
    do {
        ...
        if (theEvent.eventType == MOUSE_UP || theEvent.eventType == MOUSE_ENTERED_CELL) {
            ...
            const boolean sidebarTouchEnabled = false;
#else
            const boolean sidebarTouchEnabled = true;
#endif
            if (sidebarTouchEnabled
                && theEvent.param1 >= 0
...
void throwCommand(item *theItem, boolean autoThrow) {
    ...
    //
    if (theItem == NULL) {
        if (useQuickTargetSelection) {
            theItem = promptForQuickTargetItem();
        } else {
            theItem = promptForItemOfType((ALL_ITEMS), 0, 0,
    ...
    // Staffs and wands use Brogue's Apply/Zap path; only projectiles continue
    // through the physical Throw path below.
    if (useQuickTargetSelection
        && (theItem->category == STAFF || theItem->category == WAND)) {
        apply(theItem);
        return;
...
static boolean useStaffOrWand(item *theItem) {
    ...
    temporaryMessage("Select a direction or tap a target.", REFRESH_SIDEBAR);
    itemName(theItem, buf2, false, false, NULL);
