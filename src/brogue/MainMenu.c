...
static void titleMenu() {
    ...
    // The animated logo and native Android menu now share one title scene.
    initializeTitleFlames(true, flameColors, flameColorStorage,
                          flameColorSources, flameState, flameMask);
    drawTitleFlames(flameState, flameMask);
    updateScreen();
    ...
    while (rogue.nextGame == NG_NOTHING) {
        if (isApplicationActive()) {
            ...
            if (pauseBrogue(MENU_FLAME_UPDATE_DELAY,
                    (PauseBehavior){.interuptForMouseMove = true})) {
                // Touches outside the Android buttons do not dismiss the menu.
